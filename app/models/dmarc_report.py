...
class DmarcReport(Base):
    ...
    reporting_org: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)
    reporting_email: Mapped[Optional[str]] = mapped_column(String(320), nullable=True)

    # Time range
    period_begin: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True, index=True)
    period_end: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True, index=True)

    # From XML: <policy_published>
    policy_domain: Mapped[Optional[str]] = mapped_column(String(255), nullable=True, index=True)
    policy_adkim: Mapped[Optional[str]] = mapped_column(String(1), nullable=True)  # r or s
    policy_aspf: Mapped[Optional[str]] = mapped_column(String(1), nullable=True)   # r or s
    policy_p: Mapped[Optional[str]] = mapped_column(String(20), nullable=True)     # none/quarantine/reject
    policy_sp: Mapped[Optional[str]] = mapped_column(String(20), nullable=True)
    policy_pct: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    policy_fo: Mapped[Optional[str]] = mapped_column(String(20), nullable=True)
...
class DmarcRecord(Base):
    ...
    header_from: Mapped[Optional[str]] = mapped_column(String(255), nullable=True, index=True)
    envelope_from: Mapped[Optional[str]] = mapped_column(String(320), nullable=True)
...
class DmarcAuthResult(Base):
    ...
    selector: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)  # DKIM only
    scope: Mapped[Optional[str]] = mapped_column(String(20), nullable=True)      # SPF only
